...
describe("resolvePetAnimation: action mapping reaches the real pet", () => {
  ...
  it("accepts legacy/alias mapping values", () => {
    expect(resolvePetAnimation("running", { running: "extra-action-5" }, null).animationKey).toBe("extra_action_5");
    expect(resolvePetAnimation("running", { running: "aqua-pixel" }, null).animationKey).toBe("extra_action_aqua_pixel");
  });
  ...
  it("keeps the mapped animation when a preview key is unknown", () => {
