...
export function SourcesPanel() {
  ...
  const ids = Object.keys(providers).filter((id) => id === "claude-code");
  if (ids.length === 0) {
  ...
  return (
    <div className="sources-panel">
      <p className="note sources-note">{t("doctor.backupNote", "安装 hooks 后，Claude Code 会自动将事件发送到桌宠。备份文件保存在 ~/.claude/settings.clawd-backup.json")}</p>
      {ids.map((id) => {
